...
namespace CloudFileClient.Commands.Auth
{
    ...
    public class LoginCommand : ICommand
    {
        ...
        public LoginCommand(string username, string password, LogService logService)
        {
        ...
        {
            return _packetFactory.CreateLoginRequest(_username, _password);
        }
        ...
        public async Task<CommandResult> ExecuteAsync(ClientConnection connection)
        {
            ...
            try
            {
                _logService.Info($"Attempting to login as '{_username}'...");
                ...
                if (success)
                {
                    _logService.Info($"Login successful for '{_username}' (User ID: {userId})");
                ...
                {
                    _logService.Warning($"Login failed for '{_username}': {message}");
            ...
            catch (Exception ex)
            {
                ...
                return new CommandResult($"Login failed: {ex.Message}", exception: ex);
            }
        }
    }
}